...
public class FloatingWindow1 extends LinearLayout {
    ...
    private OnMouseLongPressListener mouseLongPressListener;
    private boolean mMouseLongPressMode = false;
    ...
    private static final int LONG_PRESS_COLOR = Color.parseColor("#FF6666"); // red
    ...
    public void setMode(Mode mode) {
        if (currentMode == mode && mode == Mode.MOUSE_TOUCH) {
            // 鼠标模式已选中时再次点击 → 切换长按模式
            setMouseLongPressMode(!mMouseLongPressMode);
            return;
        }
        if (currentMode == mode) {
            // Other modes: do nothing if clicking the same mode
            return;
        }
        // 切换到其他模式时取消长按模式
        if (mMouseLongPressMode) {
            setMouseLongPressMode(false);
        }
        currentMode = mode;
        updateButtonColors();
    ...
    private void setMouseLongPressMode(boolean active) {
        if (mMouseLongPressMode == active) {
            return;
        }
        mMouseLongPressMode = active;
        updateButtonColors();

        if (mouseLongPressListener != null) {
            mouseLongPressListener.onMouseLongPressChanged(active);
        }
    }

    public boolean isMouseLongPressActive() {
        return mMouseLongPressMode;
    }
...
private void updateButtonColors() {
        ...
        // 长按模式时显示红色，普通鼠标模式显示蓝色
        if (mMouseLongPressMode) {
            setButtonBackgroundLongPress(btnMouseMode);
        } else {
    ...
    private void setButtonBackgroundLongPress(ImageButton btn) {
        GradientDrawable bg = new GradientDrawable();
        bg.setColor(LONG_PRESS_COLOR);
        bg.setCornerRadius(dpToPx(getContext(), 4));
        btn.setBackground(bg);
    }
    ...
    public void setOnMouseLongPressListener(OnMouseLongPressListener listener) {
        this.mouseLongPressListener = listener;
    }
    ...
    public interface OnMouseLongPressListener {
        void onMouseLongPressChanged(boolean active);
    }
