...
import com.limelight.binding.input.GlobalKeyboardService;
...
public class Game extends Activity implements SurfaceHolder.Callback,
    ...
    private boolean mMouseLongPressActive = false;
    ...
    private void setupFloatingWindow1() {
                ...
                switch (mode) {
                    ...
                    case MOUSE_TOUCH:
                        ...
                        for (int i = 0; i < touchContextMap.length; i++) {
                            ...
                            // 传递长按模式标志给新创建的触摸上下文
                            if (mMouseLongPressActive && touchContextMap[i] != null) {
                                touchContextMap[i].setMouseLongPressActive(true);
                            }
                        }
                        break;
                }
        ...
        // 鼠标长按模式监听器
        floatingWindow1.setOnMouseLongPressListener(new FloatingWindow1.OnMouseLongPressListener() {
            @Override
            public void onMouseLongPressChanged(boolean active) {
                setMouseLongPressActive(active);
            }
        });
...
// Destroy the capture provider
    ...
    protected void onPause() {
        // 取消注册全局键盘拦截服务
        GlobalKeyboardService.unregisterGameActivity(this);
    ...
    protected void onResume() {
        ...
        // 注册全局键盘拦截服务
        GlobalKeyboardService.registerGameActivity(this);
    ...
    private void saveUiState() {
        ...
        if (floatingWindow1 != null) {
            ...
            editor.putBoolean("fw1MouseLongPress", floatingWindow1.isMouseLongPressActive());
            ...
            streamView.post(new Runnable() {
                ...
                public void run() {
                    ...
                    if (floatingWindow1 != null) {
                        ...
                        // Restore mouse long press mode
                        boolean mouseLongPress = prefs.getBoolean("fw1MouseLongPress", false);
                        if (mouseLongPress && !floatingWindow1.isMouseLongPressActive()) {
                            // 触发长按模式（通过点击鼠标按钮切换到长按状态）
                            floatingWindow1.setMode(FloatingWindow1.Mode.MOUSE_TOUCH);
                        }
                        if (keyboardActive) {
    ...
    // Returns true if the given key code is a system modifier key
    // that should always be captured and forwarded to the host
    ...
    public boolean handleKeyDown(KeyEvent event) {
        ...
        if (!handled) {
            ...
            // Always forward modifier keys (Win/Ctrl/Alt/Shift/Fn) to host,
            // even when input is not grabbed. This prevents system shortcuts
            // like Win+Tab, Alt+Tab, etc. from leaking to Android OS.
            if (isModifierKey(event.getKeyCode())) {
                short translated = keyboardTranslator.translate(event.getKeyCode(), event.getDeviceId());
                if (translated != 0 && event.getRepeatCount() == 0) {
    ...
    /**
     * 切换鼠标长按模式。
     * 激活时：在当前位置按下鼠标左键，触摸上下文抑制右键触发；
     * 关闭时：释放鼠标左键。
     */
    private void setMouseLongPressActive(boolean active) {
        if (mMouseLongPressActive == active) {
            return;
        }
        mMouseLongPressActive = active;

        // 同步标志到触摸上下文
        for (TouchContext ctx : touchContextMap) {
            if (ctx != null) {
                ctx.setMouseLongPressActive(active);
            }
        }
