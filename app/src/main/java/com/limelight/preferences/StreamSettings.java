...
public class StreamSettings extends Activity {
    ...
    public static class SettingsFragment extends PreferenceFragment {
        ...
        public void onCreate(Bundle savedInstanceState) {
            ...
            // 全局键盘捕获 — 开启时引导用户完成两步配置
            Preference globalKeyboardPref = findPreference(PreferenceConfiguration.GLOBAL_KEYBOARD_CAPTURE_PREF_STRING);
            if (globalKeyboardPref != null) {
                globalKeyboardPref.setOnPreferenceChangeListener(new Preference.OnPreferenceChangeListener() {
                    @Override
                    public boolean onPreferenceChange(Preference preference, Object newValue) {
                        boolean enabled = (Boolean) newValue;
                        if (enabled) {
                            // Android 13+ 侧载应用：先引导用户解除"受限设置"
                            if (com.limelight.binding.input.GlobalKeyboardService.needsRestrictedSettingsPermission(getActivity())) {
                                Dialog.displayDialog(getActivity(),
                                        getResources().getString(R.string.title_checkbox_global_keyboard_capture),
                                        getResources().getString(R.string.summary_restricted_settings_guide),
                                        false);
                                com.limelight.binding.input.GlobalKeyboardService.openAppSettings(getActivity());
                            }
                            // 无障碍服务未开启：引导用户到无障碍设置页面
                            else if (!com.limelight.binding.input.GlobalKeyboardService.isServiceEnabled(getActivity())) {
                                Dialog.displayDialog(getActivity(),
                                        getResources().getString(R.string.title_checkbox_global_keyboard_capture),
                                        getResources().getString(R.string.summary_accessibility_guide),
                                        false);
                                com.limelight.binding.input.GlobalKeyboardService.openAccessibilitySettings(getActivity());
                            }
                        }
                        return true;
                    }
